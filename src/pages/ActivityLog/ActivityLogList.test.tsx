...
describe("ActivityLogList", () => {
  ...
  it("should handle auto-refresh toggle", async () => {
    ...
    vi.mocked(activityLogApi.fetchActivityLogs).mockClear();

    // Fast-forward 30 seconds
    vi.advanceTimersByTime(30000);

    await waitFor(() => {
      expect(activityLogApi.fetchActivityLogs).toHaveBeenCalledTimes(1);
    });
  ...
  it("should handle log name filter change", async () => {
    renderWithProviders();

    await waitFor(() => {
      expect(screen.getByLabelText(/log name/i)).toBeInTheDocument();
    });
    ...
    const logNameSelect = screen.getByLabelText(/log name/i);
    fireEvent.change(logNameSelect, { target: { value: "auth" } });
  ...
  it("should handle organizational unit filter change", async () => {
    renderWithProviders();

    await waitFor(() => {
      // OrganizationalUnitPicker is a Select component
    ...
    // Find and change org unit select (it's the second combobox after log name)
    const selects = screen.getAllByRole("combobox");
    const orgUnitSelect = selects[1]; // Index 1 is org unit, 0 is log name

    if (orgUnitSelect) {
      fireEvent.change(orgUnitSelect, { target: { value: "unit-1" } });

      await waitFor(() => {
        expect(activityLogApi.fetchActivityLogs).toHaveBeenCalledWith(
          expect.objectContaining({ organizational_unit_id: "unit-1", page: 1 })
        );
      });
    }
  });
