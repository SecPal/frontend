...
- Canonicalized workspace-preview API routing so both `frontend-<workspace>.preview.secpal.dev` and `<workspace>.preview.secpal.dev` fall back to `https://api-<workspace>.preview.secpal.dev` when a preview bundle leaks an empty, loopback, or SPA-host `VITE_API_URL`, and made Playwright treat both frontend host variants as workspace previews instead of silently drifting to localhost or `api.secpal.dev`.
