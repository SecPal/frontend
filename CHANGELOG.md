...
- Canonicalized frontend workspace-preview API routing so `frontend-<workspace>.preview.secpal.dev` now falls back to `https://api-<workspace>.preview.secpal.dev` when a preview bundle leaks an empty, loopback, or SPA-host `VITE_API_URL`, and made Playwright auto-target the current Polyscope workspace preview instead of silently drifting to localhost or `api.secpal.dev` when invoked inside `~/.polyscope/clones/...`.
