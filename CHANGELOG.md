...
- Followed up the logout cleanup race fix so late bootstrap, cross-tab, and BFCache barrier cleanup keeps skipping offline vault table deletion after a full logout has already started, preventing stale auth restoration from reopening the Dexie `DatabaseClosedError` window fixed in #1146.
