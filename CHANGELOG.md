...
- Restored the `assetlinks.json` build outputs at `dist/assetlinks.json` and `dist/.well-known/assetlinks.json` by switching the Vite static-copy rules back to the plugin's flattening object form (`{ stripBase: true, name: "assetlinks.json" }`), so `app.secpal.dev` no longer falls through to the SPA shell at the Digital Asset Links endpoint after the current `vite-plugin-static-copy` behavior change.
