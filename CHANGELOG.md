...
- Bumped `eslint-plugin-react-hooks` from 7.0.1 to 7.1.0 and disabled the new `react-hooks/set-state-in-effect` rule (tracked in #874) until the `void loadFn()` in `useEffect` patterns are refactored; supersedes Dependabot PR #872.
