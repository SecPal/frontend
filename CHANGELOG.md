...
- Raised the build-toolchain Node floor in `package.json` from `>=20.6.0` to `>=22.19`, matching the effective minimum already required by direct dependencies (`vite@8`, `@vitejs/plugin-react@6`, `eslint@10`, and `lighthouse@13`) and `.nvmrc`/CI (Node 22). Enabled `engine-strict=true` in `.npmrc` so unsupported Node versions fail `npm ci`/`npm install` instead of emitting silent `EBADENGINE` warnings. Closes #1221.
