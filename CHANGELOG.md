...
- Tightened the #1146 logout-cleanup follow-up so queued offline-vault table cleanups are serialized behind a shared wait target, sensitive logout teardown still runs after a best-effort vault cleanup failure, existing cross-tab skip markers survive later `clear()` calls until barrier cleanup can observe them, and the temporary skip marker is removed once broader logout cleanup finishes so offline logout state returns to the single `auth_logout_barrier` key.
